...
bandwidth <- gwr.sel(prep(model_rec), 
                ...
                adapt = TRUE, 
                gweight = gwr.Gauss) # TODO: change to bivariate likely to not let neighbourhoods further away count as much, and also make spatial lag model
```
...
gwr_grid <- expand.grid (gweight = c(gwr.Gauss, gwr.bisquare), # TODO: check if this works for varying shape of kernel
                         adapt = c(10, 50, 100, 500)) # TODO: set this based on how many neighbourhoods we have # TODO can delete this
...
OOF_preds <- rbind(
  ...
  #                  data.frame(gwr_best_OOF_preds %>% dplyr::select(.pred,turnout_ratio),model = "Model-2a_Geographically Weighted Regression"),
  #                  data.frame(gwr_best_OOF_preds_spatial %>% dplyr::select(.pred,turnout_ratio),model = "Model-2b_Geographically Weighted Regression spatial"),
...
lm_val_fit_geo <- lm_best_wf %>%
  last_fit(split     = data_split,
           #control   = control,
           metrics   = metric_set(yardstick::rmse, yardstick::rsq)) # Without spatial cross-validation
lm_val_fit_geo_spatial <- lm_best_wf_spatial %>%
  last_fit(split     = data_split,
           #control   = control,
           metrics   = metric_set(yardstick::rmse, yardstick::rsq)) # With spatial cross-validation
# 
# # Geographically weighted regression
...
val_preds <- rbind(
  ...
  #                  data.frame(dplyr::select(gwr_val_pred_geo, .pred, turnout_ratio), model = "Model-2a_Geographically Weighted Regression"),
  #                  data.frame(dplyr::select(gwr_val_pred_geo_spatial, .pred, turnout_ratio), model = "Model-2b_Geographically Weighted Regression spatial"),
...
lm_val_fit_geo %>% pluck(".workflow", 1) %>% extract_fit_engine() %>% car::vif() # Without spatial cross validation
```

```{r}
# Check multicollinearity with vif of car package
lm_val_fit_geo_spatial %>% pluck(".workflow", 1) %>% extract_fit_engine() %>% car::vif() # With spatial cross validation
```
...
# Without spatial cross-validation
predictors <- train.set %>% select(all_of(c("bev_dichth_km2", "man", "vrouw", "p_0_15_jaar", "p_15_25_jaar", "p_25_45_jaar", "p_45_65_jaar", "p_65_jaar", "p_ongehuwd", "p_gehuwd", "p_gescheiden", "p_verweduwd", "p_ia_west", "p_ia_n_west", "aantal_bedrijf", "gem_woning_waarde", "aant_ink_ont", "gem_ink_p_o", "p_laag_ink", "p_hoog_ink", "aow_uitk", "afst_levensm_5km", "voortg_ond_3km", "tot_uitkering"))) 

explainer_RF <- DALEXtra::explain_tidymodels(
  model = rf_val_fit_geo$.workflow[[1]],
  data = predictors,
  y = as.integer(train.set$turnout_ratio*100), # modelStudio wants integers so convert fractions to percentages
  label = "Random Forest",
  verbose = FALSE
)
