...
newnames <- c('ID', 'wijkcode', 'gem_naam', 'bev_dichth_km2', 'aantal_inw', 'man', 'vrouw', 
             ...
             'turnout_ratio', 'dist_NN_voting_station', 'geometry') # Note that tot_uitkering doesn't include aow_uitk, just different ones that are e.g. because of not being able to work. Further, p_adults combines 'p_15_25_jaar', 'p_25_45_jaar', 'p_45_65_jaar', and 'p_65_jaar'. Also, n_turnout is just the raw number of people having voted in the neighbourhood
...
```{r}
# Drop variables that aren't relevant due to multicollinearity with another variable or due to many missing values (which can't be reliably imputed) # TODO: drop this and only after drop the NA valued rows!!
...
# TODO: remove ones without turnout and decide which variables to drop
...
d_NN_voting_station_plot <- ggplot(data = neighbourhood_data) +
  ...
  xlab("Distance to nearest voting station") +
  theme_classic()
