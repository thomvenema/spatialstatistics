...
lm_check_autocorrelation <- function(fit_model, weight_matrix, dataset, n_permutations=2999, suffix=''){
  ...
  mc_global <- moran.mc(fit_model$residuals, weight_matrix, n_permutations, alternative="greater")
  ...
  dataset[col_str] <- residuals(fit_model)
  lm_res <- qtm(dataset, "res_lm")
  lm_res 
...
```{r}
# Inspect spatial autocorrelation when linear regression coefficients were determined without spatial cross correlation
lm_res <- lm_check_autocorrelation(fit_model=lm_val_fit_geo, weight_matrix=adjacency_nbq_w, dataset=neighbourhood_data, n_permutations=2999, suffix='')
```

```{r}
# Inspect spatial autocorrelation when linear regression coefficients were determined with spatial cross correlation
lm_res_spatial <- lm_check_autocorrelation(fit_model=lm_val_fit_geo_spatial, weight_matrix=adjacency_nbq_w, dataset=neighbourhood_data, n_permutations=2999, suffix='spatial')
```
