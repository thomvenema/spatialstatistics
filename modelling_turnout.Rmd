...
turnout_plot <- ggplot(data = neighbourhood_data) +
  geom_density(alpha=0.8, colour="black", fill="lightblue", aes(x = to_rati)) +
...
pop_density_plot <- ggplot(data = neighbourhood_data) +
  geom_density(alpha=0.8, colour="black", fill="green3", aes(x = bv_dc_2)) +
...
plot_grid(turnout_plot, pop_density_plot, labels = "AUTO")
```
...
model_eq <- to_rati ~ bv_dc_2 + antl_nw + man + vrouw + p_0_15_ + p_15_25 + p_25_45 + p_45_65 + p_.65_j + p_nghwd + p_gehwd + p_gschd + p_vrwdw + p_i_wst + p__n_ws + antl_bd + gm_wn_w + ant_nk_ + gm_nk__ + p_lg_nk + p_hg_nk + aow_utk + afst__5 + vrtg__3 + tot_utk
...
```
```{r}
# View optimum bandwidth
bandwidth # TODO: check if this value makes sense
```
```{r}
...
# Without spatial cross-validation
predictors <- train.set %>% select(all_of(c("bv_dc_2", "antl_nw", "man", "vrouw", "p_0_15_", "p_15_25", "p_25_45", "p_45_65", "p_.65_j", "p_nghwd", "p_gehwd", "p_gschd", "p_vrwdw", "p_i_wst", "p__n_ws", "antl_bd", "gm_wn_w", "ant_nk_", "gm_nk__", "p_lg_nk", "p_hg_nk", "aow_utk", "afst__5", "vrtg__3", "tot_utk"))) 
