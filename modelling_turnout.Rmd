...
# Further, due to multicollinearity (high VIF after running linear regression) drop the following predictors: p_verweduwd, p_ongehuwd, p_gehuwd (correlation maybe with an age group),
# afst_levensm_5km (correlation maybe with population density), p_hoog_ink (insignificant in lm and correlation maybe with gem_woning_waarde), p_adults (correlation with other age groups),
# p_aow (correlation with 65+ age group)
# TODO: remove these also from recipes
neighbourhood_data <- subset(neighbourhood_data, select=-c(aantal_inw, n_turnout, gem_ink_p_o, man))

# Set a maximum value for variables to tackle outliers for more robust training
# This is based on the density plots
# TODO: do this for all relevant variables, and if its final density plot or spatial distribution map will be used in report, then compute those again too (like done above, but maybe with automatic range determining in maps) 
...
neighbourhood_data$p_65_jaar <- ifelse(neighbourhood_data$p_65_jaar <=0.3, neighbourhood_data$p_65_jaar, 0.3)
...
neighbourhood_data$tot_uitkering <- ifelse(neighbourhood_data$tot_uitkering<=0.2, neighbourhood_data$tot_uitkering, 0.2)
...
model_eq_all <- turnout_ratio ~ bev_dichth_km2 + vrouw + p_0_15_jaar + p_15_25_jaar + p_25_45_jaar + p_45_65_jaar + p_65_jaar + p_ongehuwd + p_gehuwd + p_gescheiden + p_verweduwd + p_ia_west + p_ia_n_west + aantal_bedrijf + gem_woning_waarde + aant_ink_ont + p_laag_ink + p_hoog_ink + aow_uitk + afst_levensm_5km + voortg_ond_3km + tot_uitkering + p_adults + dist_NN_voting_station

# Socio-economic variables
model_eq_economic <- turnout_ratio ~ gem_woning_waarde + aant_ink_ont + p_laag_ink + p_hoog_ink + aow_uitk + tot_uitkering

# Socio-demographic variables
model_eq_demographic <- turnout_ratio ~ vrouw + p_0_15_jaar + p_15_25_jaar + p_25_45_jaar + p_45_65_jaar + p_65_jaar + p_ongehuwd + p_gehuwd + p_gescheiden + p_verweduwd + p_ia_west + p_ia_n_west + p_adults

# Spatial location variables
model_eq_location <- turnout_ratio ~ bev_dichth_km2 + aantal_bedrijf +  afst_levensm_5km + voortg_ond_3km + dist_NN_voting_station

# Composite of most promising variables # TODO: determine all of these equations, make recipes of them and train all models for each
...
# All predictor variables
model_rec_all <- create_recipe(model_eq=model_eq_all, training_data=train.set)
...
train_GWR <- function(data, model_equation, weight_func=gwr.Gauss, bandwidth=NULL){
  # Before doing analysis for GWR the sf polygon object must be converted into a sp spatial object because the spgwr package usually works on sp objects # TODO Instead of as_Spatial (which somehow doesn't work on prepared_data) can also supply coords (the coordinates of centroids here) 
  ...
  gwr_model <- gwr(model_equation, data = data,
                   longlat = TRUE, gweight = weight_func, hatmatrix=TRUE, se.fit=TRUE, adapt=bandwidth)#bandwidth = bandwidth) # TODO: bandwidth parameter is for fixed kernel, adapt for adaptive (was the whole problem)
...
```{r}
# TODO: fix errors that are due to having too many variables to fit on too little data by training for just social factors (and gotten rid of multicolineary factors) --> on smaller equation still doesn't work (for Gaussian weight), likely due to multicollinearity and some predictors not having scaled the outliers to a maximum value (this is done for turnout already though), so remove those predictors and scale outliers
# TODO: possible also add a SGWR and spatial lag/error model to allow some variables to be stationary and hence reducing model complexity
gwr_model <- train_GWR(data=neighbourhood_data, model_equation=model_eq_all, weight_func=gwr.Gauss, bandwidth=NULL)
```
...
rf_grid <- expand.grid (mtry = c(3,6,9), 
                       min_n = c(100,500,800)) # TODO: set min_n based on how many neighbourhoods and mtry based on number of variables we have
```
...
# Inspect more model statistics 
# With spatial cross validation
tidy(lm_model_spatial)
augment(lm_model_spatial)
glance(lm_model_spatial)
```
...
lm_res_adj_lst <- lm_check_autocorrelation(fit_model=lm_model, weight_matrix=Wadj, dataset=neighbourhood_data, test_data=test.set, training_data=train.set, test_predictions=lm_val_pred_geo, n_permutations=1000, suffix='')
lm_res_adj_moran <- lm_res_adj_lst[[1]]
lm_res_adj <- lm_res_adj_lst[[2]]
```
...
lm_res_ids_lst <- lm_check_autocorrelation(fit_model=lm_model, weight_matrix=Wids, dataset=neighbourhood_data, test_data=test.set, training_data=train.set, test_predictions=lm_val_pred_geo, n_permutations=1000, suffix='')
lm_res_ids_moran <- lm_res_ids_lst[[1]]
lm_res_ids <- lm_res_ids_lst[[2]]
```
...
lm_res_ids2_lst <- lm_check_autocorrelation(fit_model=lm_model, weight_matrix=Wids2, dataset=neighbourhood_data, test_data=test.set, training_data=train.set, test_predictions=lm_val_pred_geo, n_permutations=1000, suffix='')
lm_res_ids2_moran <- lm_res_ids2_lst[[1]]
lm_res_ids2 <- lm_res_ids2_lst[[2]]
```
...
```{r}
# Inspect spatial autocorrelation when linear regression coefficients were determined without spatial cross correlation, using inverse distance squared matrix
lm_res_expdis_lst <- lm_check_autocorrelation(fit_model=lm_model, weight_matrix=Wexpdis, dataset=neighbourhood_data, test_data=test.set, training_data=train.set, test_predictions=lm_val_pred_geo, n_permutations=1000, suffix='')
lm_res_expdis_moran <- lm_res_expdis_lst[[1]]
lm_res_expdis <- lm_res_expdis_lst[[2]]
```
...
lm_res_adj_spatial <- lm_check_autocorrelation(fit_model=lm_model_spatial, weight_matrix=Wadj, dataset=neighbourhood_data, test_data=test.set, training_data=train.set, test_predictions=lm_val_pred_geo_spatial, n_permutations=1000, suffix='spatial')
```
...
estimate_error_lag_model <- function(model_equation, data, weight_matrix, model_str='error', n_permutations=1000, suffix=''){
    # Use the create recipe function on the full dataset (instead of only training data as with other models) to normalize, center and scale the predictors for more robust training
  model_rec <- create_recipe(model_eq = model_equation, training_data = data)
  prepared_data <- juice(prep(model_rec))
  
  # TODO: if this model should actually first be trained on only training data then use code of below
  # # Retrieve model equation from recipe
  # model_eq <- formula(prep(model_recipe))
  # 
  # # Retrieve the prepared data from the model recipe, which already  normalized, centered and scaled the predictors for more robust training
  # prepared_data <- juice(prep(model_recipe))
  
  # Estimate Spatial error or lag model with weight matrix
  if (model_str=='error'){
    model <- errorsarlm(model_equation, data=prepared_data, listw=weight_matrix, zero.policy=TRUE)
  }
  else if (model_str=='lag'){
    model <- lagsarlm(model_equation, data=prepared_data, listw=weight_matrix, zero.policy=TRUE)
  }
...
error_model_economic_ids_res # TODO show error residuals map
```
...
mapview::mapview(neighbourhood_data, zcol = "amb_localR2", col.regions=brewer.pal(9, "RdYlGn")) 
```
...
```{r}
min(gwr_out[,"gem_woning_waarde_se"])
```

```{r}
# TODO: get a variable that actually exists in the recipe the model is trained on, and run this function for multiple others too
gwr_lst_female <- map_coefficient_var_sig(data=neighbourhood_data, gwr_results=gwr_out, predictor="gem_woning_waarde", suffix="")
gwr_coef_female <- gwr_lst_female[[1]]
gwr_sig_female <- gwr_lst_female[[2]]
```
...
rf_model <- lm_val_fit_geo %>% pluck(".workflow", 1) %>% extract_fit_engine() 
tidy(rf_model)
augment(rf_model)
glance(rf_model)
```
...
# Inspect more model statistics 
# With spatial cross validation
rf_model_spatial <- lm_val_fit_geo_spatial %>% pluck(".workflow", 1) %>% extract_fit_engine() 
tidy(rf_model_spatial)
augment(rf_model_spatial)
glance(rf_model_spatial)
```
