...
# TODO: drop more after checking multicollinearity further, e.g. keep p_adults and drop age groups, drop aow or over 65, merge some marriage values or only keep p_gehuwd
...
# Set a maximum value for variables to tackle outliers for more robust training # TODO: do this for all relevant variables, and if its final density plot or spatial distribution map will be used in report, then compute those again too (like done above, but maybe with automatic range determining in maps) 
...
model_eq_final <- turnout_ratio ~ bev_dichth_km2 + vrouw + p_0_15_jaar + p_15_25_jaar + p_25_45_jaar + p_45_65_jaar + p_65_jaar + p_ongehuwd + p_gehuwd + p_gescheiden + p_verweduwd + p_ia_west + p_ia_n_west + aantal_bedrijf + gem_woning_waarde + aant_ink_ont + p_laag_ink + p_hoog_ink + aow_uitk + afst_levensm_5km + voortg_ond_3km + tot_uitkering + p_adults + dist_NN_voting_station
```
...
train_GWR <- function(data, model_equation, weight_func=gwr.Gauss){
  ...
  # Before doing analysis for GWR the sf polygon object must be converted into a sp spatial object because the spgwr package usually works on sp objects # Instead of as_Spatial (which somehow doesn't work on prepared_data) can also supply coords (the coordinates of centroids here) # TODO: change this or keep it
  #data_sp <- as_Spatial(prepared_data)
  data_sp <- prepared_data
  
  # Select optimal bandwidth for adaptive kernel (so k nearest neighbourhoods)
  bandwidth <- gwr.sel(model_equation, 
                data = data_sp,
                coords = cbind(data$X_centroid, data$Y_centroid),
  ...
  gwr_model <- gwr(model_equation, data = data_sp, coords = cbind(data$X_centroid, data$Y_centroid), longlat = TRUE, gweight = weight_func, hatmatrix=TRUE, se.fit=TRUE, bandwidth = bandwidth)
...
gwr_model <- train_GWR(data=neighbourhood_data, model_equation=model_eq_economic, weight_func=gwr.bisquare)#gwr.Gauss)
```
...
gwr_lst_female <- map_coefficient_var_sig(data=neighbourhood_data, gwr_results=gwr_out, predictor="vrouw", suffix="")
gwr_coef_female <- gwr_list_female[[1]]
gwr_sig_female <- gwr_list_female[[2]]
```
...
gwr_coef_female | gwr_sig_female
```
